...
namespace FSI_Project
{
		...
		if (cell->at_boundary(face_no))
		{
			if (fluid_boundaries[cell->face(face_no)->boundary_indicator()]==Interface)
			{
				...
				for (unsigned int q=0; q<n_face_q_points; ++q)
				{
				  ...
				  {
					  local_rhs(i) += (std::fabs(fe_face_values[velocities].value (i, q)*
								error))
			  ...
			  for (unsigned int j=0; j<dofs_per_cell; ++j)
				{
				  ...
				  local_matrix(i,j) += ( physical_properties.rho_f/time_step*phi_u[i]*phi_u[j]
						  	  	  	  	+ theta * ( 2*physical_properties.viscosity*symgrad_phi_u[i] * symgrad_phi_u[j]
										- div_phi_u[i] * phi_p[j]
										- phi_p[i] * div_phi_u[j]) // conditioning
  ...
  void FSIProblem<dim>::setup_system ()
  {
	...
	for (unsigned int i=0; i<4; ++i)
	{
		if (i<2) fluid_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,Neumann));
    ...
    for (timestep_number=1, time=time_step;
         time<fem_properties.T;++timestep_number)
      {
        ...
        while (true)
        {
			...
			if (count >= fem_properties.max_optimization_iterations || velocity_jump < sqrt(time_step)) break;
