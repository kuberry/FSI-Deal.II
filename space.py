...
time_mult=12000;
final_time=1;
optimization_times_time_error=5;
...
for i in range(loops):
    # RESET TIME
    n_steps = init_space_mult * pow(space_power,i);
    ...
    for key, line in enumerate(lines):
        if line[0]!='#':
            ...
            if success:
                lines[key] = 'set number of time steps  = ' + str(time_mult)
            success = re.search("jump tolerance", line)
            if success:
                lines[key] = 'set jump tolerance  = ' + str(float(optimization_times_time_error*final_time)/time_mult)
            success = re.search("nx", line) or re.search("ny", line)
            if success:
                objMatch=re.match( r'(.* = )(.*)', line, re.M|re.I)
                lines[key] = objMatch.group(1) + str(n_steps)
