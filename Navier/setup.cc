...
void FSIProblem<dim>::dirichlet_boundaries (System system, Mode enum_, bool special_case)
{
  ...
  if (physical_properties.simulation_type==4) min_index=1;
  if (enum_==state)
    {
      if (system==Fluid)
	{
...
void FSIProblem<2>::setup_system ()
{
  ...
  //******************************************************************
    ...
    fluid_triangulation.refine_global(fem_properties.num_mesh_refinements);
    structure_triangulation.refine_global(fem_properties.num_mesh_refinements);
	...
	if (i>=1 && i<=3) structure_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,Interface)); // Interface
  ...
  // std::vector<Point<2> > fluid_face_centers, temp_structure_face_centers(structure_triangulation.n_active_cells());
  ...
  // std::vector<Point<2> > structure_face_centers(fluid_interface_faces.size());
  ...
  std::vector<unsigned int> fluid_block_component (2+1,0);
  fluid_block_component[2] = 1;
  DoFRenumbering::component_wise (fluid_dof_handler, fluid_block_component);

  std::vector<unsigned int> structure_block_component (2*2,0);
  for (unsigned int i=2; i<2*2; ++i)
    structure_block_component[i] = 1;
  DoFRenumbering::component_wise (structure_dof_handler, structure_block_component);

  std::vector<unsigned int> ale_block_component (2,0);
...
Point<3> rotate_about_y(const Point<3> &input) {
  Tensor<2,3> R;
  {
    const double PI = 3.14159265358979323846;
    const double angle = 1.5*PI;
    ...
    const Point<3> axis(0,1.0,0.0);//std::sqrt(2.0),std::sqrt(2.0)); // rotate about the y axis
    ...
    //      }
    // };
    R = Tensor<2,3>(rotation);
  }
  return Point<3>(input * R);//output;
}
...
void FSIProblem<3>::setup_system ()
{
  ...
  if (physical_properties.simulation_type == 0) {
    ...
    Point<3> structure_bottom_left(0,0,fem_properties.fluid_height),
      structure_top_right(fem_properties.structure_width,fem_properties.structure_width,fem_properties.fluid_height+fem_properties.structure_height);
    std::vector<double> x_scales(fem_properties.nx_f,fem_properties.fluid_width/((double)fem_properties.nx_f));
    std::vector<double> f_y_scales(fem_properties.ny_f,fem_properties.fluid_height/((double)fem_properties.ny_f));
    std::vector<double> s_y_scales(fem_properties.ny_s,fem_properties.structure_height/((double)fem_properties.ny_s));

    std::vector<std::vector<double> > f_scales(3),s_scales(3);
    f_scales[0]=x_scales;f_scales[1]=x_scales;f_scales[2]=f_y_scales;
    s_scales[0]=x_scales;s_scales[1]=x_scales;s_scales[2]=s_y_scales;
    GridGenerator::subdivided_hyper_rectangle (fluid_triangulation,f_scales,fluid_bottom_left,fluid_top_right,false);
    GridGenerator::subdivided_hyper_rectangle (structure_triangulation,s_scales,structure_bottom_left,structure_top_right,false);
  ...
  //******************************************************************
    ...
    // 						Point<3> ( std::sqrt(0.125), std::sqrt(0.125), 5.0));//,

    // GridGenerator::subdivided_hyper_rectangle ( temp_fluid_triangulation,
    // 						repetitions,
    // 						Point<2> (-std::sqrt(0.125),-std::sqrt(0.125)),
    // 						Point<2> ( std::sqrt(0.125), std::sqrt(0.125)));//,
    //  the lower one in x-direction is 0, the upper one is 1 (the left and the right vertical face). The indicators for the surfaces in y-direction are 2 and 3, the ones for z are 4 and 5
    // GridGenerator::hyper_ball ( temp_fluid_triangulation,
    // 				Point<2> (0.0,0.0),
    // 				0.5); // radius = 1. 
    //const HyperBallBoundary<2> cylinder_boundary_description(Point<2>(0.0,0.0), 0.5); // radius, axis
    // const CylinderBoundary<3> cylinder_boundary_description(0.5, 2);
    // const HyperBallBoundary<2> top_boundary_description(Point<2>(0.0,0.0), 0.5);//
    // const HyperBallBoundary<2> bottom_boundary_description(Point<2>(0.0,0.0), 0.5);//
    //temp_fluid_triangulation.set_boundary (0, cylinder_boundary_description); // left
    //temp_fluid_triangulation.set_boundary (1, cylinder_boundary_description); // middle
    //temp_fluid_triangulation.set_boundary (2, cylinder_boundary_description); // right
    //temp_fluid_triangulation.set_boundary (3, cylinder_boundary_description); // right
    
    //temp_fluid_triangulation.refine_global (2);

    //temp_fluid_triangulation.set_boundary (0); // left
    //temp_fluid_triangulation.set_boundary (1); // middle
    ...
    // 					   fluid_triangulation );
    // fluid_triangulation.set_boundary (0, cylinder_boundary_description); // left
    // fluid_triangulation.set_boundary (1, bottom_boundary_description);
    // // fluid_triangulation.set_boundary (2, top_boundary_description);
	...
	if (i==1) fluid_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,Neumann)); // neumann
	else if (i==2) fluid_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,DoNothing));// do nothing
	else fluid_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,Interface)); // interface
	fluid_interface_boundaries.insert(3);
      }

    for (unsigned int i=1; i<=4; ++i)
      {
	// guess inner is 3, outer is 4, bottom is 1, and top is 2
	if (i==1 || i==2) structure_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,Dirichlet));
	else if (i==3) structure_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,Interface)); //interface
	else structure_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,DoNothing)); 
	structure_interface_boundaries.insert(3);
      }
    for (unsigned int i=1; i<=3; ++i)
      {
...
template <int dim>
void FSIProblem<dim>::initialize_sizes ()
{
  std::cout << "Number of active cells: "
	    << "fluid: " << fluid_triangulation.n_active_cells()
	    << " structure: " << structure_triangulation.n_active_cells()
  ...
  structure_dof_handler.distribute_dofs (structure_fe);
  ale_dof_handler.distribute_dofs (ale_fe);
  std::vector<unsigned int> fluid_block_component (4,0);
  fluid_block_component[3] = 1;
  DoFRenumbering::component_wise (fluid_dof_handler, fluid_block_component);

  std::vector<unsigned int> structure_block_component (6,0);
  for (unsigned int i=3; i<6; ++i)
    structure_block_component[i] = 1;
  DoFRenumbering::component_wise (structure_dof_handler, structure_block_component);

  std::vector<unsigned int> ale_block_component (3,0);
  DoFRenumbering::component_wise (ale_dof_handler, ale_block_component);
...
template void FSIProblem<3>::initialize_sizes ();
