...
namespace FSI_Project
{
                                             ...
                                             const unsigned int component) const
  {
	...
	if (physical_properties.moving_domain)
	{
		unit_normal_f[0]=2*sin(t)*x;
		unit_normal_f[1]=1+2*sin(t)*(1+x);
	}
	else
	{
		unit_normal_f[0]=0;
...
//	if (physical_properties.moving_domain)
//	{
//	    double n=physical_properties.n_fourier_coeffs;
//		const double t = this->get_time();
//		const double x = p[0];
//		const double y = p[1];
...
//			double jpi=j*pi;
//			double k1=cos_1*pow(-1,std::fabs(j))-1; // integral from 0 to 1 of sin*cos(n*pi*x)
//			double k2=sin_1*pow(-1,std::fabs(j)+1); // integral from 0 to 1 of cos*cos(n*pi*x)
//			double k_j1=(cos_t*k1+sin_t*k2)/(pow(jpi,2)-1);
//			double k_j2=(cos_t*k2-sin_t*k1)/(pow(jpi,2)-1);
//
//			if (j==0)
//			{
//		    	F[0][1]+=k_j1;
...
//			else
//			{
//		    	F[0][0]-=k_j1*jpi*sin(jpi*x)*sinh(jpi*y)/sinh(jpi);
//		    	F[1][0]-=k_j2*jpi*sin(jpi*x)*sinh(jpi*y)/sinh(jpi);
//		    	F[0][1]+=k_j1*jpi*cos(jpi*x)*cosh(jpi*y)/sinh(jpi);
//		    	F[1][1]+=k_j2*jpi*cos(jpi*x)*cosh(jpi*y)/sinh(jpi);
//			}
//	    }
//	    F[0][0]*=sin(1+t);
//	    F[1][0]*=cos(1+t);
//	    F[0][1]*=sin(1+t);
//	    F[1][1]*=cos(1+t);
//		detTimesFinv[0][0]=1+F[1][1];
//		detTimesFinv[0][1]=-F[0][1];
//		detTimesFinv[1][0]=-F[1][0];
//		detTimesFinv[1][1]=1+F[0][0];
//		determinant = std::fabs(detTimesFinv[0][0]*detTimesFinv[1][1]-detTimesFinv[0][1]*detTimesFinv[1][0]);
//		pval = 2*physical_properties.viscosity*((sin(x + t)*sin(y + t) - cos(x + t)*cos(y + t))*F[1][1]+(-cos(x+t)*cos(y+t)+sin(x+t)*sin(y+t))*F[1][0]) + 2./determinant*physical_properties.mu*cos(x + t)*sin(y + t);
//
//	}
//	else
//	{
...
//		detTimesFinv[1][0]=0;
//		detTimesFinv[1][1]=1;
//		pval = 2*physical_properties.mu*cos(t + x)*sin(t + y) - 2*physical_properties.viscosity*(cos(t + x)*cos(t + y)
//					- sin(t + x)*sin(t + y));
//	}

	grad_u=1./determinantJ*grad_u*detTimesFinv;
	pval = physical_properties.viscosity*((grad_u+transpose(grad_u))*unit_normal_f)*unit_normal_f+physical_properties.mu*((grad_n+transpose(grad_n))*unit_normal_s)*unit_normal_f;
	//pval = 40*x*y*cos(t);
	switch (component)
	{
	case 0: // NEED UNIT NORMAL VECTOR!
		result[0]=2*physical_properties.viscosity*grad_u[0][0]-pval;
		result[1]=2*physical_properties.viscosity*0.5*(grad_u[0][1]+grad_u[1][0]);
		return result;
	case 1:
		result[0]=2*physical_properties.viscosity*0.5*(grad_u[0][1]+grad_u[1][0]);
...
//				double k_j1=(cos_t*k1+sin_t*k2)/(pow(jpi,2)-1);
//				double k_j2=(cos_t*k2-sin_t*k1)/(pow(jpi,2)-1);
//				double z_j1=(-sin_t*k1+cos_t*k2)/(pow(jpi,2)-1);
//				double z_j2=(-sin_t*k2-cos_t*k1)/(pow(jpi,2)-1);
//
//				if (j==0)
//				{
//			    	F[0][1]+=k_j1;
...
//			    	F[0][1]+=k_j1*jpi*cos(jpi*x)*cosh(jpi*y)/sinh(jpi);
//			    	F[1][1]+=k_j2*jpi*cos(jpi*x)*cosh(jpi*y)/sinh(jpi);
//			    	Z[0]+=cos(jpi*x)*sinh(jpi*y)/sinh(jpi)*(z_j1*sin(1+t)+k_j1*cos(1+t));
//			    	Z[1]+=cos(jpi*x)*sinh(jpi*y)/sinh(jpi)*(z_j2*cos(1+t)-k_j2*sin(1+t));
//				}
//		    }
//		    F[0][0]*=sin(1+t);
//		    F[1][0]*=cos(1+t);
...
//			detTimesFinv[1][1]=1+F[0][0];
//			determinant = std::fabs(detTimesFinv[0][0]*detTimesFinv[1][1]-detTimesFinv[0][1]*detTimesFinv[1][0]);
//			grad_u = 1./determinant * grad_u * detTimesFinv;
//
...
////			1./determinant*detTimesFinv[k][0]*(hessian_u[i][0][0]*1./determinant*detTimesFinv[j][0]+hessian_u[i][1][0]*1./determinant*detTimesFinv[j][1])
////																+1./determinant*detTimesFinv[k][1]*(hessian_u[i][0][1]*1./determinant*detTimesFinv[j][0]+hessian_u[i][1][1]*1./determinant*detTimesFinv[j][1]);
//
//			p_y1 =2*physical_properties.viscosity*(cos(t + x)*sin(t + y) + cos(t + y)*sin(t + x))*(F[1][1]+F[0][1]) - 2./determinant*physical_properties.mu*sin(t + x)*sin(t + y);
//			p_y2 =2*physical_properties.viscosity*(cos(t + x)*sin(t + y) + cos(t + y)*sin(t + x))*(F[1][1]+F[0][1]) + 2./determinant*physical_properties.mu*cos(t + x)*cos(t + y);
//		}
//		else
//		{
//			detTimesFinv[0][0]=1;
...
//			p_y1 = 2*physical_properties.viscosity*(cos(t + x)*sin(t + y) + cos(t + y)*sin(t + x)) - 2*physical_properties.mu*sin(t + x)*sin(t + y);
//			p_y2 = 2*physical_properties.viscosity*(cos(t + x)*sin(t + y) + cos(t + y)*sin(t + x)) + 2*physical_properties.mu*cos(t + x)*cos(t + y);
//			Z=0;
//		}
	Tensor<2,dim> F;
	F[0][0]=2*sin(t)*(1+x);
...
//	double derivative_inv_surf_determinant = -0.5*pow(1./surf_determinant,3)*(2*(grad_n[1][0]*hessian_n[1][0][0] + (1+grad_n[0][0])*hessian_n[0][0][0]));
//	unit_normal_f_y1=unit_normal_f*derivative_inv_surf_determinant;
//	unit_normal_f_y1[0]+=-1./surf_determinant*hessian_n[1][0][0];
//	unit_normal_f_y1[1]+=1./surf_determinant*hessian_n[0][0][0];
	...
	const double u2_t=2*sin(t + x)*sin(t + y) - 2*cos(t + x)*cos(t + y); 
	Tensor<1,dim> Z;
	...
	case 2:
//		if (std::fabs(grad_u[0][0]+grad_u[1][1])>1e-17)
//			std::cout << grad_u[0][0]+grad_u[1][1] << std::endl;
		return 0;//-(grad_u[0][0]+grad_u[1][1]);
	default:
		return 0;
	}
  }
...
//				else
//				{
//					F[0][0]-=k_j1*jpi*sin(jpi*x)*sinh(jpi*y)/sinh(jpi); // all written in Jacobian form (transposed)
//					F[1][0]-=k_j2*jpi*sin(jpi*x)*sinh(jpi*y)/sinh(jpi);
//					F[0][1]+=k_j1*jpi*cos(jpi*x)*cosh(jpi*y)/sinh(jpi);
//					F[1][1]+=k_j2*jpi*cos(jpi*x)*cosh(jpi*y)/sinh(jpi);
//				}
...
//			determinant = std::fabs(detTimesFinv[0][0]*detTimesFinv[1][1]-detTimesFinv[0][1]*detTimesFinv[1][0]);
//
//			return 2*physical_properties.viscosity*((sin(x + t)*sin(y + t) - cos(x + t)*cos(y + t))*F[1][1]+(-cos(x+t)*cos(y+t)+sin(x+t)*sin(y+t))*F[0][1]) + 2./determinant*physical_properties.mu*cos(x + t)*sin(y + t);
//    	}
//    	else
//    	{
...
//    	}
                                      ...
                                      const unsigned int component) const
  {
    ...
    // All wrong since based on 3 dirichlet + 1 neumann b.c.
//    double n=physical_properties.n_fourier_coeffs;
//	const double t = this->get_time();
//	const double x = p[0];
//	const double y = p[1];
...
//		else
//		{
//			ans[0]+=k_j1*cos(jpi*x)*sinh(jpi*y)/sinh(jpi);
//			ans[1]+=k_j2*cos(jpi*x)*sinh(jpi*y)/sinh(jpi);
//		}
//    }
//    F[0][0]*=sin(1+t);
//    F[1][0]*=cos(1+t);
...
//    }
        ...
        if (component==0)
        {
        	return 2*sin(t)*(x+0.5*pow(x,2)-0.5*pow(y,2));//0.5*pow(y,2);//*sin(t);x+y;
        }
        else
        {
        	return 2*sin(t)*(y+0.5*pow(y,2)-0.5*pow(x,2));//0.5*pow(x,2)+3*y;//*cos(t)0.5*(x+y);
        }
  }
  template <int dim>
  void
                                        ...
                                        Vector<double>   &values) const
{
  ...
  double FSIProblem<dim>::interface_error()
  {
	...
	return sqrt(temp_vector.l2_norm()+temp2_vector.l2_norm());
  }
  ...
  void FSIProblem<dim>::build_dof_mapping()
  {
	...
	for (; cell!=endc; ++cell)
	{
	 ...
	 //for (unsigned int i=0;i<f_a.size();++i) std::cout << f_a[i].dof << " " << f_a[i].coord << " " << f_a[i].component << std::endl;
	}
	{
	typename DoFHandler<dim>::active_cell_iterator
		  ...
		  for (unsigned int i=0; i<dofs_per_cell; ++i)
			{
			  ...
			  local_rhs(i) += (physical_properties.rho_f/time_step *phi_i_s*old_u*determinantJ
								...
								* fe_values.JxW(q);
			  local_rhs(i) += theta*phi_i_s*rhs*fe_values.JxW(q)*determinantJ+theta*p_i_s*div_u*fe_values.JxW(q)*determinantJ;
			  local_rhs(i) += (1-theta)*phi_i_s*old_rhs*fe_values.JxW(q)*determinantJ;
			}
		}
				...
				if (cell->at_boundary(face_no))
				  {
					if (fluid_boundaries[cell->face(face_no)->boundary_indicator()]==Neumann)
					{
						...
						for (unsigned int q=0; q<n_face_q_points; ++q)
						{
						  ...
						  unit_normal[0]=-(1+2*sin(time)*(1+fe_face_values.quadrature_point(q)[1])); // negative of 1+n1y
						  unit_normal[1]=-2*sin(time)*fe_face_values.quadrature_point(q)[1]; // n2y
						  ...
						  for (unsigned int i=0; i<dofs_per_cell; ++i)
						  {
							  ...
							  {
								  local_rhs(i) += multiplier*(fe_face_values[velocities].value (i, q)*
											new_stresses*unit_normal * surf_determinant
											*  fe_face_values.JxW(q));
							  }
						  }
						}
		  ...
		  for (unsigned int q=0; q<n_q_points; ++q)
			{
			  ...
			  double determinantJ = (1+F[q][0][0])*(1+F[q][1][1])-F[q][0][1]*F[q][1][0];
			  Tensor<2,dim> detTimesFinv;
			  detTimesFinv[0][0]=1+F[q][1][1];
			  detTimesFinv[0][1]=-F[q][1][0]; // taking care of the transpose between gradient and Jacobian
			  detTimesFinv[1][0]=-F[q][0][1]; // same
			  detTimesFinv[1][1]=1+F[q][0][0];
			  for (unsigned int k=0; k<dofs_per_cell; ++k)
				{
				  phi_u[k]		   = fe_values[velocities].value (k, q);
				  grad_phi_u[k] = fe_values[velocities].gradient (k, q);
				  ...
				  for (unsigned int j=0; j<dofs_per_cell; ++j)
					{
					  ...
					  local_matrix(i,j) += ( physical_properties.rho_f/time_step*phi_u[i]*phi_u[j]
											+ theta * ( 2*physical_properties.viscosity
												*0.25/determinantJ*scalar_product(grad_phi_u[i]*detTimesFinv+transpose(grad_phi_u[i]*detTimesFinv),grad_phi_u[i]*detTimesFinv+transpose(grad_phi_u[i]*detTimesFinv))
											- div_phi_u[i] * phi_p[j]
											- phi_p[i] * div_phi_u[j]) // conditioning [this is okay since rhs = 0  for this test function]
  ...
  void FSIProblem<dim>::dirichlet_boundaries (System system, Mode enum_)
  {
	...
	if (enum_==state)
	{
		if (system==Fluid)
		{
			...
			{
				if (fluid_boundaries[i]==Dirichlet || fluid_boundaries[i]==Interface)// || fluid_boundaries[i]==Neumann)// ||
				{
...
//					else if (ale_boundaries[i]==Interface)
//					{
//						AleBoundaryValues<dim> ale_boundary_values_function(physical_properties);
//						ale_boundary_values_function.set_time (time);
//						VectorTools::interpolate_boundary_values (ale_dof_handler,
//																  i,
//																  ale_boundary_values_function,
//																  ale_dirichlet_boundary_values,
//																  ale_fe.component_mask(ale_displacement));
//					}
			}
  ...
  void FSIProblem<dim>::setup_system ()
  {
	...
	for (unsigned int i=0; i<4; ++i)
	{
		...
		else if (i==1) structure_boundaries.insert(std::pair<unsigned int, BoundaryCondition>(i,Neumann));
  ...
  void FSIProblem<dim>::compute_error ()
  {
	...
	VectorTools::integrate_difference (ale_dof_handler, solution.block(2), ale_exact_solution,
				 ...
				 VectorTools::L2_norm);
	std::cout << "ALE error:" <<  ale_cellwise_errors.l2_norm() << std::endl;
	...
	//for (unsigned int i=0; i<fluid_cellwise_errors.size();++i) std::cout << fluid_cellwise_errors[i] << std::endl;
  ...
  void FSIProblem<dim>::run ()
  {
    ...
    VectorTools::project(structure_dof_handler, structure_constraints, QGauss<dim>(fem_properties.structure_degree+2),
                          structure_boundary_stress,
                          old_stress.block(1));
    transfer_interface_dofs(old_stress,old_stress,1,0);
    old_stress*=-1; //the fluid is expecting the solution as the negative of the stress from the structure


    stress=old_stress;

    // we feed this in so that the ALE solver can get an initial mesh
    {
		solution=old_solution;
		assemble_ale(state);
		dirichlet_boundaries(ALE,state);
		solve(ALE,state);
		transfer_all_dofs(solution,old_mesh_displacement,2,0);
		mesh_displacement.block(0)=old_mesh_displacement.block(0);
    }

    // delete this all later
	AleBoundaryValues<dim> ale_boundary_values(physical_properties);
	AleVelocityBoundaryValues<dim> ale_velocity_boundary_values(physical_properties);
	ale_boundary_values.set_time(0);
	VectorTools::project (ale_dof_handler, ale_constraints, QGauss<dim>(fem_properties.ale_degree+2),
						  ale_boundary_values,
						  mesh_displacement.block(2));
	transfer_all_dofs(mesh_displacement,mesh_displacement,2,0);
    ...
    for (timestep_number=1, time=time_step;
         timestep_number*time_step<=fem_properties.T;++timestep_number)
      {
        ...
        {
        	++count;
// REMARK: Uncomment to feed the true stress solution coming from the structure domain
//			fluid_boundary_stress.set_time(time);
//			VectorTools::project(fluid_dof_handler, fluid_constraints, QGauss<dim>(fem_properties.fluid_degree+2),
//								  fluid_boundary_stress,
//								  stress.block(0));
//			transfer_interface_dofs(stress,stress,0,1);
			structure_boundary_stress.set_time(time);
			VectorTools::project(structure_dof_handler, structure_constraints, QGauss<dim>(fem_properties.structure_degree+2),
								  structure_boundary_stress,
								  stress.block(1));
			transfer_interface_dofs(stress,stress,1,0);
			stress*=-1;

			// Solve for the state variables
			structure_boundary_values.set_time(time);
			VectorTools::project (structure_dof_handler, structure_constraints, QGauss<dim>(fem_properties.structure_degree+2),
								  structure_boundary_values,
								  solution.block(1));
			assemble_ale(state);
            ...
            // Currently we project the mesh_displacement for a given transformation
			old_mesh_displacement.block(0)=mesh_displacement.block(0);
        	ale_boundary_values.set_time(time);
        	ale_velocity_boundary_values.set_time(time);
			VectorTools::project (ale_dof_handler, ale_constraints, QGauss<dim>(fem_properties.ale_degree+2),
								  ale_boundary_values,
								  mesh_displacement.block(2));
			solution=mesh_displacement; // NEEDS REMOVED!!! just for plotting purpose
        	transfer_all_dofs(mesh_displacement,mesh_displacement,2,0);
...
//			VectorTools::project (ale_dof_handler, ale_constraints, QGauss<dim>(fem_properties.ale_degree+2),
//								  ale_velocity_boundary_values,
//								  mesh_velocity.block(2));
//			transfer_all_dofs(mesh_velocity,mesh_velocity,2,0);
			...
			if (count >= fem_properties.max_optimization_iterations) break;
